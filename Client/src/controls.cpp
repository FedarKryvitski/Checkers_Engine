...
Board::Board() {
	static const std::array<std::string, 5> texturesPaths = {"Image/best.png", "Image/blunder.png", "Image/forced.png",
															 "Image/good.png", "Image/inaccurasy.png"};
...
void Board::setPosition(Vector2f position) { Object::setPosition(position); }
...
void Board::flip() {
	...
	std::for_each(field, field + 8, [](auto row[]) { std::reverse(row, row + 8); });
	flipped = !flipped;
}
...
void Label::setText(const std::string &_text) { text.setString(_text); }
...
void Label::setTextThickness(float thick) { text.setOutlineThickness(thick); }

void Label::setFontSize(int fontSize) { text.setCharacterSize(fontSize); }

void Label::setTextColor(Color color) { text.setFillColor(color); }

void Label::setOutlineColor(Color color) { text.setOutlineColor(color); }
...
void Object::setColor(Color color) { background.setFillColor(color); }

void Object::setThickness(float thickness) { background.setOutlineThickness(thickness); }
...
void Object::setVisible(bool _visible) { visible = _visible; }
...
bool Clickable::isPressed(Vector2i mousePosition) const {
	...
	return visible && ((posx - x) >= 0) && (posx - x <= width) && ((posy - y) >= 0) && (posy - y <= height);
}

void Clickable::setOnPress(const std::function<void()> &callback) { onPressFunction = callback; }
...
Button::Button(const std::string &_caption, Vector2f _position) : Button() {
	caption.setString(_caption);
	setPosition(_position);
}

Button::Button(const std::string &_caption, Vector2f _position, Vector2f _size) : Button(_caption, _position) {
	setSize(_size);
}
...
void Button::onRelease() { background.setFillColor(Color::Green); }
...
void Choice::setStatus(bool _status) { status = _status; }

bool Choice::getStatus() { return status; }
...
void Bar::setFirstColor(Color color) { first.setFillColor(color); }

void Bar::setSecondColor(Color color) { second.setFillColor(color); }
...
inline void AssessBar::setTextPosition() {
	...
	const float newY = value < 0 ? y + 4 : y + height - fontSize - 4;
...
void CommentSection::setValues(std::vector<StateData> &vdata) {
...
void CommentSection::draw(RenderWindow &window) const {
	Object::draw(window);
	std::ranges::for_each(vText, [&window](const auto &elem) { window.draw(elem); });
}
...
void Input::onPress() { isSelected = visible; }
...
void Input::onRelease() { isSelected = false; }
...
void Input::setSize(Vector2f _size) { Clickable::setSize(_size); }

void Input::setLimit(int _maxTextLength) { maxTextLength = _maxTextLength; }

std::string Input::getText() { return text.getString(); }
...
void TWait::setPosition(Vector2f _position) { Object::setPosition(_position); }
...
void TWait::draw(RenderWindow &window) const {
	if (visible) {
		std::ranges::for_each(circlesArray, [&window](const auto &elem) { window.draw(elem); });
	}
}
